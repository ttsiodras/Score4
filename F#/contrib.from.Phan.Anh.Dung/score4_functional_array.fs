...
let rec abMinimax maximizeOrMinimize color depth (board:Cell array) =
    ...
    | _  ->
        let boards = validMoves |> Array.map (fun column -> dropDisk board column color) in
        let scores = Array.map (fun board -> scoreBoard board) boards in
        let killerMoveIndex =
        ...
        | None ->
            let bestScores =
                ...
                | 1 -> scores
                | _ ->
                    boards |> Array.map (fun board ->let _, bscore = abMinimax (not maximizeOrMinimize) (enum (- int color)) (depth-1) board
                                                     match bscore with
